...
export default function Blog() {
  ...
  return (
    <div>
      <TitleSection
        title="Tattoo Universe Blog: Insights, Trends, and Tips"
        subtitle="Explore our latest blog posts for in-depth guides, emerging trends, and essential tips for tattoo enthusiasts. For more articles and expert advice, visit our full blog page."
      />
      <div className="mx-auto mt-12 sm:mt-18 max-w-7xl px-4">
        <div className="sm:flex sm:items-baseline sm:justify-between">
          <Link href="/pages/blogs/blogpage" className="hidden text-sm font-semibold text-gray-100 hover:text-gray-400 sm:block">
            See all blog
            <span aria-hidden="true"> &rarr;</span>
          </Link>
        </div>

        <div className="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:grid-rows-2 sm:gap-x-6 lg:gap-8">
          {blogPosts.map((post) => (
            <div key={post.id} className={`group aspect-h-1 aspect-w-2 overflow-hidden rounded-lg ${post.aspectClass}`}>
              ...
              <div className="flex items-end p-6 sm:absolute sm:inset-0">
                <div className="bg-black bg-opacity-50 shadow-xl w-full p-4 rounded" data-aos="fade-up">
                  <h3 className="font-semibold text-gray-100 text-shadow-black">
                    <Link href={post.link}>
                      <span className="absolute inset-0" />
                      {post.title}
                    </Link>
                  </h3>
                  <p aria-hidden="true" className="mt-1 text-sm text-pink-200 text-shadow-black">
                    {post.description}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-6 sm:hidden">
          <Link href="/pages/blogs/blogpage" className="block text-sm font-semibold text-gray-100 hover:text-gray-400">
            See all blog
            <span aria-hidden="true"> &rarr;</span>
          </Link>
        </div>
